# Task 2 — Read disturb on v(Q), and MA widened to 0.24 µm

## Does v(Q) move during the read?

Yes. The node holding the `0` is pulled up the moment the wordline turns on
(Fig. A2).
...
MA1 connects the precharged bitline to Q against the MN1 pull-down; Q settles
where the two currents balance. The peak is at the wordline edge, when BL is
still at full V_DD. The decay afterwards is not the latch restoring itself — it
tracks the bitline collapsing toward v(Q), which removes the pull-up. Because
the bitline here is a floating capacitor, the disturb is self-limiting; with a
...
bitlines clamped at V_DD (Fig. A3): RSNM falls 138.2 → 81.4 mV, a **41 %
collapse**, against a ≈ V_DD/6 ≈ 183 mV rule-of-thumb target. The cell is one
coupling event, supply droop, or V_t mismatch from losing its state.
...
with clamped bitlines, with pull-downs cut to CR 0.25, or down to V_DD = 0.5 V.
...
SNM is the largest axis-aligned square inscribed in the lobe, found by bisection
in `plots.py`. Read trip points (527 / 537 mV) exceed the hold value (493 mV)
because the access device loads QB.
